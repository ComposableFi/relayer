...
func (sp *SubstrateProvider) QueryHeaderAtHeight(ctx context.Context, height int64) (ibcexported.ClientMessage, error) {
	...
	latestBeefyBlockHash, err := sp.LightClient.RPC.Beefy.GetFinalizedHead()
	if err != nil {
		return nil, err
	}

	latestBeefyHeight, err := sp.LightClient.RPC.Chain.GetBlock(latestBeefyBlockHash)
	if err != nil {
		return nil, err
	}

	if height > int64(latestBeefyHeight.Block.Header.Number) {
		return nil, fmt.Errorf("queried block is not finalized")
	}

	blockHash, err := sp.LightClient.RPC.Chain.GetBlockHash(uint64(height))
	if err != nil {
		return nil, err
	}
